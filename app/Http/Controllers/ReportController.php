...
class ReportController extends Controller
{
    ...
    */
    ...
    /**
     * Generates a user report for mobile without performing a POST request.
     * This uses the defaults and doesn't allow for any customization.
     *
     * @param Illuminate\Http\Request $request
     * @return Illuminate\Http\Response
     */
    public function generateMobileReportPDF(Request $request)
    {
        $userReport = \App\Models\SurveyUserReport::find($request->link);
        ...
        // Use the default data generated by the reports route.
        // HTML extraction is so volatile :(
        $resp = $this->showUserReport($request)->render();
        preg_match('/<!-- START:\.page -->(.+)<!-- END:\.page -->/s', $resp, $matches);
        $data = trim(substr(trim($matches[1]), 28, -6));

        $req = Request::create(URL::to('/survey/generate-user-report'), 'POST', [
            'userLink'      => $request->link,
            'htmlContent'   => $data,
        ]);
        
        return $this->createUserPDF($req);
    }
